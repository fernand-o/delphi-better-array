...
type
  ...
  IReadOnlyList<T> = interface(IEnumerable<T>)
    ...
    function FirstIndexOf(const Value: T): Integer;
...
function TDUnitXList<T>.FirstIndexOf(const Value: T): Integer;
var
  i: Integer;
begin
  for i := 0 to GetCount - 1 do
    if GetComparer.Compare(FList[i], Value) = 0 then
      Exit(i);
  Result := -1;
end;
...
function TDUnitXList<T>.GetComparer: IComparer<T>;
begin
